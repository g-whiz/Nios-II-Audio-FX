...
#define WAVE_HEIGHT 12000
#define WAVE_FREQUENCY 100
...
void sbuf_update_out_samples(void) {
  ...
  for (; num_samples > 0; num_samples--) {
    ringmod_sample.left = (int) sbuf->buf[sbuf->idx_out].left;
    ringmod_sample.right = (int) sbuf->buf[sbuf->idx_out].right;
